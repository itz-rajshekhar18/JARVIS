...
from Automation_playmusic_Sfy import play_music_on_spotify;
...
def Auto_main_Brain(text):
    try:
        if text.startswith("open"):
            ...
            FAST_DF_TTS.speak("which song do you wanna play sir !!!")
            clear_file()
            output_text = ""
            while True :
            ...
            text = text.strip()
            t1 = threading.Thread(target=speak,args=(f"doing research 'bout {text}",))
            ...
            text = text.replace("saerch in google","")
            t1 = threading.Thread(target=speak,args=(f"performing research 'bout {text} in google search engine",))
